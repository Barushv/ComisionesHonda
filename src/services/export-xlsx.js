...
export function exportXLSX() {
  ...
  const sheetQ1 = ventas.map((v) => ({
    ...
    Banco: v.banco || "",
    Tramo: tramo,
    // >>> importante: pasamos v.version para detectar HEV por versión
    ComVehiculo: getVehiculoComision(v.modeloReal, tramo, tablas, v.version),
    ComFinanciamiento: getFinanciamientoComision(
      v.modeloReal,
      v.pago,
      v.banco,
      tablas,
  ...
  const sheetQ2 = ventas.map((v) => ({
    ...
    Seguro$: getSeguroMonto(v.modeloReal, v.seguroAnios, tablas, v.version),
    GarantiaAnios: v.garantiaAnios,
    Garantia$: getGarantiaMonto(
      v.modeloReal,
      v.garantiaAnios,
      tablas,
      v.version
    ...
    Accesorios10: Math.round((v.accesoriosMonto || 0) * 0.1),
